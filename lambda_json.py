import json
import boto3
import traceback
from decimal import Decimal
from datetime import datetime

# AWS Clients
s3 = boto3.client("s3")
dynamodb = boto3.resource("dynamodb")

TABLE_NAME = "clean_records"
table = dynamodb.Table(TABLE_NAME)


def get_risk_level(magnitude):
    if magnitude >= 5:
        return "High"
    elif magnitude >= 3:
        return "Medium"
    else:
        return "Low"


def lambda_handler(event, context):
    print("CI/CD Deployment Verification Successful")

    total_records = 0
    inserted_records = 0
    rejected_records = 0

    try:

        bucket = event["Records"][0]["s3"]["bucket"]["name"]
        key = event["Records"][0]["s3"]["object"]["key"]

        print(f"Bucket : {bucket}")
        print(f"File   : {key}")

        # Process only JSON files
        if not key.lower().endswith(".json"):
            print("Skipped - Not a JSON file")
            return {
                "statusCode": 200,
                "body": json.dumps("Ignored - Not a JSON file")
            }

        response = s3.get_object(Bucket=bucket, Key=key)

        # Convert JSON float -> Decimal
        data = json.loads(
            response["Body"].read().decode("utf-8"),
            parse_float=Decimal
        )

        features = data.get("features", [])

        print(f"Total Features : {len(features)}")

        for feature in features:

            total_records += 1

            earthquake_id = feature.get("id")
            props = feature.get("properties", {})

            magnitude = props.get("mag")
            place = props.get("place")
            timestamp = props.get("time")

            if earthquake_id is None or magnitude is None:
                rejected_records += 1
                continue

            magnitude = Decimal(str(magnitude))

            item = {
                "record_id": str(earthquake_id),
                "place": str(place) if place else "Unknown",
                "magnitude": magnitude,
                "risk_level": get_risk_level(float(magnitude)),
                "file_type": "json",
                "event_time": datetime.utcfromtimestamp(
                    timestamp / 1000
                ).strftime("%Y-%m-%d %H:%M:%S UTC"),
                "processed_at": datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S UTC")
            }

            table.put_item(Item=item)

            inserted_records += 1

        print("========== JSON ETL SUMMARY ==========")
        print(f"Total Records    : {total_records}")
        print(f"Inserted Records : {inserted_records}")
        print(f"Rejected Records : {rejected_records}")

        return {
            "statusCode": 200,
            "body": json.dumps({
                "message": "JSON ETL Completed Successfully",
                "total": total_records,
                "inserted": inserted_records,
                "rejected": rejected_records
            })
        }

    except Exception as e:

        print("========== ERROR ==========")
        print(str(e))
        traceback.print_exc()

        return {
            "statusCode": 500,
            "body": json.dumps(str(e))
        }